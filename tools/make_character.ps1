...
$outDir = Join-Path $root "assets\characters\player_hero"

if (-not (Test-Path $Blender)) {
    throw "Blender not found at $Blender. Pass -Blender <path to blender.exe>."
}

# A full render replaces the character outright. Layer files are discovered by
# name, so a layer an older version wrote and this one does not -- the first
# hero had a weapon_back sheet -- would otherwise be drawn on top forever.
...
$args = @("--background", "--factory-startup", "--python",
          (Join-Path $PSScriptRoot "blender_character.py"), "--") + $Only + @("--out", $outDir)
# Blender writes progress to stderr, which Windows PowerShell turns into error
# records that "Stop" treats as fatal. Only the exit code decides.
